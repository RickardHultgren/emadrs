...
\item What advantages and disadvantages are identified from a professional clinical perspective, using a digital mood evaluation instrument for depression in screening and follow-up?
\item What staff categories would be most affected by digital questionnaires?
\item The aim is also to collect proposals for further development of e{\sc madrs}.
\end{enumerate}
...
\section*{Results}
Around the example e{\sc madrs} the following potentials, strengths and weaknesses were identified:
\begin{itemize}
...
\item These questionnaires (in previous code) should be connected with each other in a controlled way.\\{\it Code name in appendix: {\bfseries emadrs\_in\_dev\_controll}}
...
In an effective organisation the managing process is distinguished from the leadership \cite{leader1}. Below is a table describing what different staff categories are managing or leading in their work according to the interviews:
\nopagebreak
\begin{table}
...
\end{table}
\newpage
Digital questionnaires are a way to {\it manage the communication with the patient}. According to the table above, the staff categories that are working with that task are: nurses, administrators and psychotherapists. Thus those staff categories would probably be most affected by the use of digital questionaires.
...
In order to foresee shifts in work load that the new digital tools could lead to, further scientific work has to be done. This is important since economic benefits could only be gained if the work tasks would change, since the finances are a mirroring of what work is actuallty done.
